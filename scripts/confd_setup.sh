...
touch /etc/confd/conf.d/env.toml
touch /etc/confd/templates/env.tmpl
...
keys = [
  "/fail2banconfig"
]' > /etc/confd/conf.d/fail2banconfig.toml

echo $fail2banconfig > /etc/confd/templates/fail2banconfig.toml


read -r -d '' spark_env <<'EOF'
#!/usr/bin/env bash
...
keys = [
  "/spark_env"
]' > /etc/confd/conf.d/spark_env.toml
